...
class AvlTree
{
  public:
    ...
    // Additional public method for solving problems by Kazuki Sona
    /**
  ...
  private:
    ...
    // ======== Internal private methods ==============
     bool remove(const Comparable &x, AvlNode * &t, int &numRemoveCalls)
     {
        if( t == nullptr )
     ...
     bool find(AvlNode * &t, const Comparable &x, int &numCalls)
     {
        if (t == nullptr)
            return false;
        else if( x < t->element )
     ...
     int totalPathLength(AvlNode * &t, int sum)
     {  
        if (t == nullptr)
            return 0;
